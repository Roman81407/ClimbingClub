...
public class Controller {
    ...
    void executeSelectQuery(ActionEvent event) {
        try (Connection connection = Main.getMyConnection()) {
            ...
            if (value == 1) {
                String query = "SELECT climbersingroups.ID_Group, FIO, Date_S, Date_E, Leader FROM climbersingroups " +
                        "INNER JOIN climbers ON climbers.ID_Climber = climbersingroups.ID_Climber " +
                        "INNER JOIN teams ON teams.ID_Group = climbersingroups.ID_Group"; // Запрос 2
                ResultSet rs = statement.executeQuery(query);
                while (rs.next()) {
                    System.out.println(rs.getInt("climbersingroups.ID_Group"));
                    System.out.println(rs.getString("FIO"));
            ...
            if (value == 2) {
                String query = "SELECT Name, (SELECT COUNT(ID_Climber) FROM climbersingroups " +
                        "WHERE climbersingroups.ID_Group = climbing.ID_Group) AS Количество FROM climbing " +
            ...
            if (value == 3) {
                String query = "SELECT ID_Group, FIO FROM climbersingroups " +
                        "INNER JOIN climbers ON climbers.ID_Climber = climbersingroups.ID_Climber " +
